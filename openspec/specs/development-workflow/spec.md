...
How spec changes become shipped code in this repo: Dan edits specs in pull requests, Claude implements them on the same branch, and Dan reviews and merges.
...
- **WHEN** Claude pushes implementation commits to the pull request
- **THEN** no new implementation run starts

### Requirement: Implementation lands on the same pull request
Claude SHALL implement a spec pull request on its own branch, so spec and code are reviewed and merged together.
...
Claude SHALL tell Dan when a pull request is ready for review.
...
- **WHEN** Claude cannot finish
- **THEN** the pull request gets a comment explaining the blocker and the `blocked` label

### Requirement: Owner tasks
Anything Claude cannot do itself SHALL be tracked as a GitHub issue labelled `owner-task`, with exact steps.
...
Dan SHALL be able to ask for changes by commenting `@claude` on a pull request or issue.

#### Scenario: Follow-up on a pull request
- **WHEN** Dan comments `@claude` with a request on a pull request
- **THEN** Claude makes the change on that pull request's branch and replies
